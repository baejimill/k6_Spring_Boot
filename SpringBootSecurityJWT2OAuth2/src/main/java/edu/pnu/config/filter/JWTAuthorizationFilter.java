...
public class JWTAuthorizationFilter extends OncePerRequestFilter{
	...
	//인가 설정을 위해 사용자의 Role 정보를 앍어 들이기 위한 객체 설정
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
		...
		String jwtToken = srcToken.replace("Bearer", "");          //토큰에서"Bearrer"를 제거
		
		//코튼에서 username 추출
		String username = JWT.require(Algorithm.HMAC256("edu.pnu.jwt")).build().verify(jwtToken).getClaim("username").asString();
		
		Optional<Member> opt = memberReopsitory.findById(username);   //코튼에서 얻은 username으로 DB를 검색해서
		...
		//DB애서 읽은 사용자 정보를 이용해서 UserDetails 타입의 객체를 생성
